...
def Date_validation(date: datetime):
    if date > datetime.date.today():
        raise ValidationError(
            _("Data nie może być nowsza niż dzisiejsza data."))
...
def Price_validation(price):
    ...
    if round(price, 2) != price:
        raise ValidationError(
            _("Cena może być sprecyzowana do jednego grosza."))
...
class Book(models.Model):
    ...
    author = models.ForeignKey(
        'Author', on_delete=models.SET_NULL, null=True, validators=[Name_validation])
    ...
    genre_genre = models.ForeignKey(
        'Genre', on_delete=models.SET_NULL, null=True,  validators=[Name_validation])
    section_section = models.ForeignKey(
        'Section', on_delete=models.SET_NULL, null=True,  validators=[Name_validation])
...
class Order(models.Model):
    ...
    client_client = models.ForeignKey(
